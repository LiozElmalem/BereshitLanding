	...
	public void loop() {
		...
		while(bs.getAlt() > Moon.realDestinationPoint.y && bs.getLat() < Moon.realDestinationPoint.x) {
			...
			// lower than 2 km - horizontal speed should be close to zero
				...
				bs.setNN(0.5); // brake slowly, a proper PID controller here is needed!          (P I D 0 1)
//				bs.setNN(bs.pid.control(bs.dt, 1 - bs.getNN()));
			...
			bs.setAlt(bs.getAlt() - bs.getDT() * bs.getVS()); // y
			bs.setLat(bs.getLat() + bs.getDT() * bs.getHS()); // x
			bs.setDist(new Point(bs.getLat(), bs.getAlt()).distance2D(Moon.realDestinationPoint));
